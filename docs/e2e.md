...
| 2026-09-08 | Umbau: Extension gestrichen, Proxy-Regel als PAC ueber GNOME | `uni-vpn setup` erneut: "Proxy-Regel im System eingetragen", `gsettings` zeigt `mode 'auto'` und die PAC-URL, Doctor "Proxy-Regel: System liest http://127.0.0.1:1081/proxy.pac". Zwei Fehler dabei gefunden und behoben: `systemctl enable --now` liess den alten Daemon-Code laufen (Installer startet jetzt neu); ein Unit-Test rief `sysproxy.install` ungefaelscht auf und hatte die echte Einstellung schon vorher gesetzt, die Sicherung enthielt deshalb "auto" (Harness blockiert das jetzt, Sicherung von Hand auf "none" gesetzt). |
| 2026-09-08 | 6 Google Chrome 152 (headless, Wegwerf-Profil, keine Zusatzoptionen, liest GNOME) | `ifconfig.me` 212.47.181.7 (direkt), sogo-Anmeldeseite durch den Tunnel (10,4 s inklusive Aufbau bei Bedarf), Domain `ifconfig.me` ueber `/api/domains` ergaenzt: 3 s spaeter liefert ifconfig.me 129.206.196.152 (Uni), nach Ruecknahme wieder 212.47.181.7. Chrome laedt die PAC also bei Aenderung der GNOME-Einstellung sofort nach. |
| 2026-09-08 | 7 Firefox 155 (headless, frisches Profil, kein Add-on, `network.proxy.type` 5) | sogo durch den Tunnel (5,3 s), ifconfig.me direkt. Nach Domainaenderung: nach 3 s noch direkt, nach 10 s ueber die Uni; Ruecknahme ebenso. `network.proxy.socks_remote_dns` ist `false`: Firefox loest lokal auf, im Readme vermerkt. |
| 2026-09-08 | 6/7 in Davids Browsern (GUI, nach Neustart, Proxy aus GNOME) | Rueckmeldung "Klappt"; der Daemon zeigt zu dem Zeitpunkt `connected` mit einer aktiven Verbindung. |
